...
public record BankUserFormResponse(String username,
    ...
    public static List<BankUserFormResponse> toUserResponse(List<BankUserModel> users) {
        return users.stream().map(usersResponse -> new BankUserFormResponse(usersResponse.getUsername(), usersResponse.getUserEmailPixKey(), usersResponse.getUserType())).toList();
    }

    public static BankUserFormResponse toUserResponse(BankUserModel user) {
        return new BankUserFormResponse(user.getUsername(), user.getUserEmailPixKey(), user.getUserType());
    }
}
